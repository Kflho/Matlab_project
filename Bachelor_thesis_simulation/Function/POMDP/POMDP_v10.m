...
% 性能损失矩阵
C_0 = [0.002566, 0.0007018, 0.005604];      % 正常状态代价 [无, 低, 高]
C_A = [692.29, 33.2776, 248.52];             % 受攻击状态代价 [无, 低, 高]

% 探测性能参数
DetRate = [0.0, 0.45, 0.98];
...
for t = 1:SimTime
    ...
    if mod(t-1, PolicyUpdateInterval) == 0
        ...
        % [核心逻辑 A：信念集计算] (串行，涉及 unique 操作，不适合并行)
        B = [1, zeros(1, T)];
        P = zeros(T+1, T+1);
        ...
        % ===== [并行化 1] 预计算 next_idx_unobs =====
        fprintf('正在并行计算信念转移映射 (n=%d)...\n', n_beliefs);
        ...
        for iter = 1:1000
            ...
            parfor i = 1:n_beliefs
                b = B(i, :);
                imm = b(1)*C_0 + (1-b(1))*C_A;           % 期望即时代价
                q0 = imm(1) + gamma * V_old(next_idx_unobs(i));
                q_rest = imm(2:3) + gamma * V_old(1);
                [V_temp(i), best_a] = min([q0, q_rest]);
                policy_temp(i) = best_a - 1;
            end
            V = V_temp;
            policy = policy_temp;
