function Add_Threshold(threshold_value, threshold_label)
    ...
    for i = 1:length(all_axes)
        ...
        h_th = yline(ax, threshold_value, '--r', ...
            'Tag', 'ThresholdLine', 'DisplayName', latex_th_label);

        % 重建图例（简单方式，信任所有对象的 DisplayName 已正确）
        all_lines = findobj(ax, 'Type', 'line', '-or', 'Type', 'constantline');
        ...
        % 获取当前图例位置（若存在）
        old_lgd = findobj(gcf, 'Type', 'Legend', 'Axes', ax);
        if ~isempty(old_lgd)
        ...
        new_lgd = legend(ax, valid_lines, 'Location', old_loc);
