...
%    5. 对比：∀ ω，局域残差与全局残差中 ω 对应分块的最大偏差逐元素为零（运行后报告实际精度）
%
%  依赖：
...
if all_passed
    ...
    fprintf('    完全一致（最大偏差逐元素为零（运行后报告实际精度））。\n');
...
for omega = 1:n_omega
    ...
    saveas(figh_y, [out_pic sprintf('center%d_ry_comparison.png', omega)]);
    saveas(figh_y, [out_pic sprintf('center%d_ry_comparison.fig', omega)]);
    ...
    % 保存 r_s 对比图
    saveas(figh_s, [out_pic sprintf('center%d_rs_comparison.png', omega)]);
    saveas(figh_s, [out_pic sprintf('center%d_rs_comparison.fig', omega)]);
