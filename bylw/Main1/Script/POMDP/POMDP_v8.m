...
for t = 1:SimTime
    ...
    if mod(t-1, PolicyUpdateInterval) == 0
        if t > 1
            ...
            fprintf('▶ 上个周期总性能损失: %.4f\n', period_cost_sum);
            fprintf('▶ 上个周期平均单步损失 (期望): %.4f\n', period_cost_sum / PolicyUpdateInterval);
    ...
    % 【新增】：查表计算当步真实开销并累加
    if GroundTruth(t) == 1
        period_cost_sum = period_cost_sum + C_A(act + 1);
    else
        period_cost_sum = period_cost_sum + C_0(act + 1);
    end
...
fprintf('\n=== 仿真结束 (t=%d) ===\n', SimTime);
fprintf('▶ 最终阶段总性能损失: %.4f\n', period_cost_sum);
fprintf('▶ 最终阶段平均单步损失: %.4f\n\n', period_cost_sum / rem_steps);
